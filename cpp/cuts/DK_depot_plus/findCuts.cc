...
vector<CutReturn> DK_depot_plus::findCuts (vector<vector<double>> const &x)
{
  ...
  for (auto const &iComponent: components)
  {
    ...
    for (int const i: iComponent)
    {
      ...
      for (int const l: iComponent)
      {
        ...
        double const sep_dep = seperate_depots(x, i, l);
...
//           continue;
        ...
        double const phi = sep_dep + 2 * x[i][l] + x[l][i] - 2;
