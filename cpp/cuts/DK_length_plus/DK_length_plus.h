...
class DK_length_plus: public Cut
{
  ...
  public:
    ...
    DK_length_plus(int priority, bool lazy, bool user, 
...
inline bool DK_length_plus::already_in_sequence (std::vector<int> const &seq, 
                                                 int n, int b, int k) const
{
  return (b == k or find(cbegin(seq), cbegin(seq) + n, b) != cbegin(seq) + n);
}

inline bool DK_length_plus::connected_to_depot (std::vector<std::vector<double>> const &x, 
                                                int idx) const
{
...
inline bool DK_length_plus::connected_from_depot (std::vector<std::vector<double>> const &x,
                                                  int idx) const
{
  for (size_t d = 0; d < d_nDepots; ++d)
    if (x[d][idx] > gg_eps)
...
inline double DK_length_plus::sum_depots (std::vector<std::vector<double>> const &x,
